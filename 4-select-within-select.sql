...
where x.name = (select name from world y where y.continent = x.continent
...
select distinct x.name, x.continent
from world x 
where population > all
(select y.population/3 from world y where x.continent = y.continent and x.name <> y.name)
